...
class MixingNetwork(nn.Module):
    def __init__(self, n_components):
        ...
        self.hidden = nn.Linear(in_features=1, out_features=3, bias=True)
        # output layer
...
class VarianceRecurrentNetwork(nn.Module):
    ...
    def forward(self, x, hidden_state):
        x = torch.cat((x.squeeze(0), hidden_state.squeeze(0)), 0)
...
class RMDN(nn.Module):
    ...
    def forward(self, x, hidden_state):
        ...
        error = torch.pow(x - torch.matmul(eta, mu.reshape(-1, 1)), 2)
